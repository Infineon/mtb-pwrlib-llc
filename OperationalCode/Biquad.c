...
void BIQUAD_PoleZeroInit(BIQUAD_t* biquad, const float32_t fs, const float32_t k,
                         const float32_t wz[2], const float32_t wp[2])
{
    biquad->k = k;
    float32_t wz_inv[2], wp_inv[2];
...
void BIQUAD_ContinuousInit(BIQUAD_t* biquad, const float32_t fs, const float32_t a[3], const float32_t b[3])
{
...
void BIQUAD_DiscreteInit(BIQUAD_t* biquad, const float32_t fs, const float32_t m[3], const float32_t n[3])
{
    biquad->fs = fs;
...
void BIQUAD_Reset(BIQUAD_t* biquad, const float32_t output)
{
    float32_t D = output / (biquad->m[0] + biquad->m[1] + biquad->m[2]);
    for (uint8_t index = 0; index < 2; ++index)
    {
        biquad->d[index] = D;
    }
...
RAMFUNC_BEGIN
float32_t BIQUAD_Run(BIQUAD_t* biquad, float32_t input)
{
    biquad->input = input;
