...
typedef enum
{
    Init_Manual_Params = 0U,    // reinitialize manual parameters to their default values
    Auto_Calc_Params,           // auto-calculate parameters (excluding manual parameters)
    Reset_Modules,              // reset all modules after any parameter change
    Flash_Params,               // store ram parameters to flash
    Run_Sys_Id,                 // run system identification
    Max_Fcn_Count
} FCN_EXE_LABEL_t;

typedef uint32_t FCN_EXE_REG_t;
// STATIC_ASSERT(Max_Fcn_Count <= (sizeof(FCN_EXE_REG_t) * 8U),
//     "Change definition of FCN_EXE_REG_t to uint64_t or larger."); // TODO PWRSUITEFW-211
...
typedef struct
{
    ...
    FCN_EXE_REG_t done;     // indicates execution completion,      written/read by FW only
    FCN_EXE_CALLBACK_t callback[Max_Fcn_Count];     // callback functions
} FCN_EXE_HANDLER_t;

extern MULT_CORE_VLT FCN_EXE_HANDLER_t fcn_exe_handler;
