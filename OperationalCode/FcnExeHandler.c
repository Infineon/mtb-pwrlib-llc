...
MULT_CORE_SHM_SUB(".fcn_exe_handler") MULT_CORE_VLT FCN_EXE_HANDLER_t fcn_exe_handler;
...
static void InitManualWrapper()
{
    if (sm.current == Init_State) // only in init state, otherwise ignore
    {
    ...
    fcn_exe_handler.done |= FCN_EXE_HANDLER_Mask(Init_Manual_Params);
}
...
void FCN_EXE_HANDLER_Init()
{
    ...
    fcn_exe_handler.callback[Init_Manual_Params] = InitManualWrapper;
    ...
    fcn_exe_handler.callback[Run_Sys_Id] = EmptyFcn;
}
